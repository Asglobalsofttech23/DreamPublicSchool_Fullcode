  ...
  const handleSubmit = async (e) => {
    ...
    try {
      for (const data of payfee) {
        const remainingfee = calculateRemainingFees(data.van, data.vanpayFees);
        ...
        const response = await axios.post(`${config.apiURL}/feeAllocation/vanfeeslogdata`, formDataToSend);

        if (response.status === 201) {
          const { feeslogid } = response.data;
          navigate(`/vaninvoice/${feeslogid}`);
        } else {
          ...
          <table>
            ...
            <tbody>
              <tr>
                <td>1</td>
                <td>Van fees</td>
                <td><input type="text" value={data.van} readOnly /></td>
              </tr>
              <tr>
                <td></td>
                ...
                <td>
                  <input
                    type="number"
                    value={calculateRemainingFees(data.van, data.vanpayFees)}
                    readOnly
                  />
                </td>
              </tr>
...
export default VanFeeCollectionForm;
