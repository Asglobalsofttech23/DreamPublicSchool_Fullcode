...
function EcaFeeCollectionForm() {
  ...
  // Calculate remaining fees based on total fees (van) and the sum of already paid fees (ecapayFees) and current paying fee
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      for (const data of payfee) {
        const remainingfee = calculateRemainingFees(data.eca_fees, data.ecaPayFees);
        ...
        const response = await axios.post(`${config.apiURL}/feeAllocation/ecafeeslogdata`, formDataToSend);

        if (response.status === 201) {
          const { feeslogid } = response.data;
          navigate(`/ecainvoice/${feeslogid}`);
        } else {
          ...
          <table>
            ...
            <tbody>
              <tr>
                <td>1</td>
                <td>Scheme fees</td>
                <td><input type="text" value={data.eca_fees} readOnly /></td>
              </tr>
              <tr>
                <td></td>
                ...
                <td>
                  <input
                    type="number"
                    value={calculateRemainingFees(data.eca_fees, data.ecaPayFees)}
                    readOnly
                  />
                </td>
              </tr>
...
export default EcaFeeCollectionForm;
